...
from helper_func import subscribed, encode, decode, get_messages, check_token, get_token, verify_user, check_verification, howtov
    ...
    if not is_admin: 
        ...
        if not is_verified:
            btn = [[
                ...
                InlineKeyboardButton("🔻 How to open and Verify 🔺", url=await howtov())
            ]]
    ...
    if len(text)>7:
        ...
        for msg in messages:
            ...
            try:
                ...
                tt = await ss.reply_text(f"<b>𝙄𝙈𝙋𝙊𝙍𝙏𝘼𝙉𝙏  ↦↦↦⃟👉 This Movie File will be deleted in 1 Hour. So Please forward this File Before Download 📥</b>",disable_web_page_preview=True, quote=True)
